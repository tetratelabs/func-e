...
tarball_url=$(${curl} -H "Authorization: token ${GITHUB_TOKEN}" https://api.github.com/repos/tetratelabs/getenvoy/releases?per_page=10 |
...
${curl} -H "Authorization: token ${GITHUB_TOKEN}" -H'Accept: application/octet-stream' "${tarball_url}" | tar -xzf - getenvoy
./getenvoy -version
