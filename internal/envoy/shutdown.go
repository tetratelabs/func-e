...
func (r *Runtime) handleShutdown(ctx context.Context) {
	...
	_, _ = moreos.Fprintf(r.Out, "invoking shutdown hooks with deadline %s\n", deadline.Format(dateFormat))
	...
	for _, f := range r.shutdownHooks {
		...
		go func() {
			...
			if err := f(timeout); err != nil {
				_, _ = moreos.Fprintf(r.Out, "failed shutdown hook: %s\n", err)
			}
		}()
	}
	wg.Wait()
...
func (r *Runtime) interruptEnvoy() {
	p := r.cmd.Process
	_, _ = moreos.Fprintf(r.Out, "sending interrupt to envoy (pid=%d)\n", p.Pid)
