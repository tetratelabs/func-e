...
func TestParseReference(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: Reference{Flavor: "standard", Version: "1.17.1", Platform: ""},
		},
		{
			name:     "standard:1.17.1/darwin",
			input:    `standard:1.17.1/darwin`,
			expected: Reference{Flavor: "standard", Version: "1.17.1", Platform: "darwin"},
		},
		{
			name:     "standard:1.17.1/linux-glibc",
			input:    `standard:1.17.1/linux-glibc`,
			expected: Reference{Flavor: "standard", Version: "1.17.1", Platform: "linux-glibc"},
		},
		{
			name:     "wasm:1.15",
			input:    `wasm:1.15`,
...
func TestParseReferenceValidatesInput(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:       `stan dard:1.17.1/darwin`,
			expectedErr: `"stan dard:1.17.1/darwin" is not a valid GetEnvoy reference. Expected format: <flavor>:<version>[/<platform>]`,
		},
	}
...
func TestParseReferenceNormalizesStringForm(t *testing.T) {
	tests := []struct {
		...
		{
			input:    `standard:1.17.1`,
			expected: `standard:1.17.1`,
		},
		{
			input:    `standard:1.17.1/darwin`,
			expected: `standard:1.17.1/darwin`,
		},
		{
			input:    `standard:1.17.1/`,
			expected: `standard:1.17.1`,
