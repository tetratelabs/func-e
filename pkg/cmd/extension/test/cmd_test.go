...
func TestGetEnvoyExtensionTestValidateFlag(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			// Run "getenvoy extension test" with the flags we are testing
			c, stdout, stderr := cmd.NewRootCommand(&globals.GlobalOpts{})
			c.SetArgs(append([]string{"extension", "test"}, test.args...))
...
func TestGetEnvoyExtensionTestFailsOutsideExtensionDirectory(t *testing.T) {
	// Change to a non-workspace dir
	o := &globals.GlobalOpts{ExtensionDir: morerequire.RequireAbs(t, ".")}

	// Run "getenvoy extension test"
...
docker bin: %s
docker args: run -u %s --rm -e GETENVOY_GOOS=%s -t -v %s:/source -w /source --init getenvoy/extension-rust-builder:latest test
...
func TestGetEnvoyExtensionBuildWithDockerOptions(t *testing.T) {
	...
	require.NoError(t, err, `expected no error running [%v]`, c)
	require.Regexp(t, ".*--init -e VAR=VALUE -v /host:/container build/image build.*", stdout.String(), `expected stdout running [%v]`, c)
	require.Equal(t, "docker stderr\n", stderr.String(), `expected stderr running [%v]`, c)
}

// TestGetEnvoyExtensionTestFail ensures test failures show useful information in stderr
func TestGetEnvoyExtensionTestFail(t *testing.T) {
	o, cleanup := requireGlobalOpts(t)
	defer cleanup()

	// "-e exit" is a special instruction handled in the fake docker script
	toolchainOptions := "-e docker_exit=3"
	// Run "getenvoy extension test"
	c, stdout, stderr := cmd.NewRootCommand(o)
	c.SetArgs([]string{"extension", "test", "--toolchain-container-options", toolchainOptions})
	err := rootcmd.Execute(c)

	// We expect the exit instruction to have gotten to the fake docker script, along with the default options.
	expectedDockerArgs := fmt.Sprintf(`run -u %s --rm -e GETENVOY_GOOS=%s -t -v %s:/source -w /source --init %s getenvoy/extension-rust-builder:latest test`,
		builtin.DefaultDockerUser, runtime.GOOS, o.ExtensionDir, toolchainOptions)
	expectedErr := fmt.Sprintf(`failed to unit test Envoy extension using "default" toolchain: failed to execute an external command "%s %s": exit status 3`, o.DockerPath, expectedDockerArgs)
	require.EqualError(t, err, expectedErr, `expected an error running [%v]`, c)
